...
<style type="text/css">
	...
	th.th-top { padding-bottom:10px; border-bottom: 1px solid #E1E1E1;}
	td.th-middle-top{padding-bottom:10px; border-bottom: 1px solid #E1E1E1; text-align: left; color: #000000; font-size: 20px; font-weight: bold;}
	td.th-middle { color:#777777; padding:10px; border-bottom:1px solid #E1E1E1; }
	table{margin: 50px auto;}
	input{border: 1px solid #dedede; color:#565656; height: 16px; outline: none;}
...
<body>
	<div>
		...
		<form>
			<table style="margin: 0 auto;">
				<tr style="color:#777; font-size: 12px;">
					...
					<th class="w-px500 th-top" >상품명</th>
				...
				<tr>	
					<td>
					<img class="imgpath" src="<c:url value='/' />${vo.item_imgpath }" style="width:80; height: 113px;" >
					</td>
					
					<td>${vo.item_name } ${detail[0]}</td>
					<td>${vo.item_price }</td>
					<td>${detail[1]}</td>
				</tr>
				</c:forEach>
				<tr style="background-color: #F4F4F4; color:#6d6c71; font-size: 12dp; text-align: right;" >
					<td colspan="5" style="padding:10px;" class="right" >total cost : ${vo.totalPrice }</td>
				</tr>
			</table>		
			...
			<table>
				...
				<tr>
					...
					<td class="w-px500 th-middle"><input name="member_id" readonly="readonly" value="${member.member_name }"></td>
				</tr>
				<tr style="border-top: 1px solid gray">
					<td class="w-px120 th-middle">E-amil:</td>
					<td class="w-px500 th-middle"><input name="member_email" value="${member.member_email }"/></td>
				</tr>
				<tr style="border-top: 1px solid gray">
					<td class="w-px120 th-middle">연락처:</td>
					<td class="w-px500 th-middle"><input name="member_phonenum" maxlength="11" onKeyup="SetNum(this);" value="${member.member_phonenum }" /></td>
				</tr>
			...
			<table>
				...
				<tr style="border-top: 1px solid gray">
					...
					<td class="w-px500 th-middle"><input name="shipping_phonenum" /></td>
				</tr>
				<tr style="border-top: 1px solid gray">
					<td class="w-px120 th-middle">배송지:</td>
					<td class="w-px500 th-middle">	
						<input type="text" name="shipping_post" maxlength="5" style="width: 70px; margin-bottom: 10px;" readonly>
						<a class="btn-fill-s" onclick="daum_post()" >우편번호찾기</a> <br/>
						<input type="text" name="shipping_address" readonly/>
						<input type="text" name="shipping_address2" value="상세주소를 입력하세요" onfocus="this.value=''"/>
					</td>
	
				</tr>
				<tr>
					<td class="w-px120 th-middle" >배송메세지</td>
					<td class="w-px120 th-middle"><input type="text" style="width:200px;" name="shipping_message" /></td>
				</tr>
				<tr>
					<td class="w-px120 th-middle">결제수단</td>
					<td class="w-px120 th-middle">
						<label>무통장입금<input type="radio" name="pay" value="무통장입금" checked="checked"></label>
		...
		<div class="btnSet">
			<a onclick="go_pay" class="btn-fill">결제하기</a> 
...
function daum_post(){
	new daum.Postcode({
		oncomplete:function(data){
			...
			$('[name=shipping_address]').eq(0).val(member_address);	
